...
testthat::test_that(desc="recode_na_if errors on absence of args",
          ...
          testthat::expect_error(recode_na_if(some_data,"nuh",c("A2","A3"),replacement=0),
                                "All grouping_cols should exist in the data set",
                                fixed=TRUE)
          testthat::expect_error(recode_na_if(some_data,"ID",c("A2","no"),replacement=0),
                                 "All target_groups should exist in grouping_cols",
                                 fixed = TRUE)
          test_specificity <- recode_na_if(some_data,"ID",c("A2","A3"),replacement=0)
          testthat::expect_true(is.na(test_specificity[[2]][1]))
                    })
