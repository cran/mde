...
#' @param x A data.frame object with missing values
#' @param grouping_col Character column to use for grouping the data
...
#' @export
recode_na_if.data.frame <- function(x,
                         grouping_col = NULL,
                         target_groups = NULL,
                         replacement = 0){
  if(any(is.null(grouping_col),is.null(target_groups))){
