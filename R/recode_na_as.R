...
#' @description This provides a convenient way to recode "NA" as another value 
#' for instance "NaN", "n/a" or
#' any other value a user wishes to use.
#' @inheritParams recode_as_na
#' @return An object of the same type as x with NAs replaced with the 
#' desired value.
#' @examples
...
if(!all(subset_cols %in% names(df))){
  ...
  final_res<-recode_helper(df,pattern_type="contains",original_value=NA,
...
stop("pattern_type should be one of starts_with,ends_with,contains or regex")
    ...
    final_res<-recode_helper(df,pattern_type=pattern_type,
                             original_value=NA,pattern=pattern,
                  new_value=value,
                  case_sensitive=case_sensitive,...)
  }
